...
query_info = {"queries_types" : defaultdict(int), "slow_queries" : [], "query_times" : []}
...
def pytest_sessionstart(session):
    ...
    def after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
        ...
        if "SAVEPOINT" not in statement and 'pytest_pyfunc_call' in [frame.function for frame in inspect.stack(0)]:
            if time_for_query.microseconds/1000 > Config.WARN_IF_QUERIES_OVER_MS:
                query_info["query_times"].append({"query_statement" : statement, "query_time" : time_for_query.microseconds/1000})
            query_info["query_times"].append(time_for_query.microseconds/1000)
...
def pytest_terminal_summary(terminalreporter):
    ...
    terminalreporter.write_line(f"Average query time: {round(mean(query_info['query_times'] or [0]), 3)}")
