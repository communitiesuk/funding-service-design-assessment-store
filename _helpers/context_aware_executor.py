...
class ContextAwareExecutor():
    """
    This Executor coppy current flask application context and then inherit the flask application context for
    each executor thread then those threads will have the ability to use flask resources with its own flask context
    """
    def __init__(self, max_workers, thread_name_prefix, flask_app):
        """
        Initialize Threadpool executor and ContextAwareExecutor
        :max_workers number of workers for the thread pool
        :thread_name_prefix prefix of the thread pool name
        :flask_app original flask application context
        """
