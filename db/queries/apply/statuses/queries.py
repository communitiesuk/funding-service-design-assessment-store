...
from db.queries.apply.application import get_application
from db.queries.apply.feedback import retrieve_end_of_application_survey_data
from db.queries.apply.form.queries import get_form
from db.queries.apply.research import retrieve_research_survey_data
from services.apply import get_round
from services.apply.data import get_application_sections
...
def update_form_status(
    ...
    """Updates the status of a whole form based on the statuses of all questions
    within the form.
...
def _is_field_answered(field: dict) -> bool:
    """Determines whether or not an answer has been provided for the supplied
    field.
...
def _determine_answer_status_for_fields(
    ...
    """Builds a list of bools representing which fields have been answered in the
    supplied question page.
...
def _determine_question_page_status_from_answers(answer_found_list: list[bool]) -> str:
    """Determines the status of this question page (could contain multiple
    questions/responses), based on the supplied list of whether each field is
    answered.
...
def update_question_page_statuses(stored_form_json: dict):
    """Updates `status` field in every question page in this form object, based on
    whether or not answers are supplied.
...
def update_statuses(application_id: str, form_name: str, is_summary_page_submitted: bool = False):
    """Updates the status of questions, forms, and the application, based on the
    state of the supplied form. If no form supplied, just updates the status of
    the application (based on feedback and form status)
