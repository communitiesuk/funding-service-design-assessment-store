...
from services.apply.aws import list_files_by_prefix
...
def create_qa_base64file(application_data: dict, with_questions_file: bool):
    """If the query param with_questions_file is True then it will get the
    application questions ans answers, and then it will generate a formatted text
    document for an application with questions and answers.
...
def get_fund_id(application_id):
    """Function takes an application_id and returns the fund_id of that
    application."""
    try:
...
def attempt_to_find_and_update_project_name(question_json, application) -> None:
    """Updates the applications project name if the updated question_json contains
    a field_id match on the pre-configured project_name field_id."""
