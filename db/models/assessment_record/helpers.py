...
def find_answer_by_key_cof(field_key: str, app_id : str):
    ...
    return ( db.session.query(func.jsonb_path_query_first(
                json_forms,
                f"$.questions[*].fields[*] ? (@.key == \"{field_key}\")",
            )).select_from(AssessmentRecords, func.jsonb_array_elements(AssessmentRecords.jsonb_blob["forms"]).alias("value"))
    .filter(
            json_forms.contains(
