...
def get_data(endpoint: str, params: Optional[dict] = None):
    """Queries the api endpoint provided and returns a data response in json
    format.
...
def get_local_data(endpoint: str, params: Optional[dict] = None):
    ...
    if params:
        ...
        query_string = urlencode(params)
        endpoint = endpoint + "?" + query_string
    api_data_json = os.path.join(Config.FLASK_ROOT, "tests_apply", "api_data", "get_endpoint_data.json")
...
def get_account(email: Optional[str] = None, account_id: Optional[str] = None) -> Account | None:
    """Get an account from the account store using either an email address or
    account_id.
