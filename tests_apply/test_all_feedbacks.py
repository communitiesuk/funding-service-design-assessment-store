...
def test_retrieve_all_feedbacks_and_surveys(mocker, app_sections, applications, report_mapping):
    mocker.patch(
        "db.queries.apply.feedback.queries.get_application_sections",
        return_value=app_sections,
    )
    mocker.patch(
        "db.queries.apply.feedback.queries.get_applications",
        return_value=applications,
    )
    mocker.patch(
        "db.queries.apply.feedback.queries.get_report_mapping_for_round",
...
def test_api_get_all_feedbacks_and_survey_report(
    ...
    response = flask_test_client.get(
        "/apply/applications/get_all_feedbacks_and_survey_report?"
        "fund_id=test_fund&round_id=test_round&status_only=SUBMITTED",
