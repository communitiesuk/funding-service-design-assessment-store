...
from db.queries.apply.application import get_all_applications
from db.schemas import ApplicationSchema
from fsd_utils.services.aws_extended_client import SQSExtendedClient
from moto import mock_aws
from services.apply.models.fund import Fund
from services.apply.models.round import Round
from tests_apply.helpers import application_expected_data
from tests_apply.helpers import count_fund_applications
from tests_apply.helpers import expected_data_within_response
from tests_apply.helpers import get_row_by_pk
from tests_apply.helpers import key_list_to_regex
from tests_apply.helpers import post_data
from tests_apply.helpers import test_application_data
from tests_apply.helpers import test_question_data


@pytest.mark.unique_fund_round(True)
def test_create_application_is_successful(flask_test_client, unique_fund_round, mock_get_application_display_config):
    """GIVEN We have a functioning Application Store API WHEN we try to create an
    application THEN applications are created with the correct parameters."""
    ...
    response = post_data(flask_test_client, "/apply/applications", application_data_a1)
    assert response.status_code == 201
    assert response.json()["language"] == "en"
    count_fund_applications(flask_test_client, unique_fund_round[0], 1)
    response = post_data(flask_test_client, "/apply/applications", application_data_a2)
...
def test_create_application_language_choice(
    ...
    mocker.patch("apply.api.routes.application.routes.get_fund", return_value=mock_fund)
    blank_forms_mock = mocker.patch(
        "apply.api.routes.application.routes.get_blank_forms",
        return_value=MagicMock(),
    )
    test_application = Applications(**application_expected_data[2])
    create_application_mock = mocker.patch(
        "apply.api.routes.application.routes.create_application",
        return_value=test_application,
    )
    mocker.patch("apply.api.routes.application.routes.add_new_forms")
...
def test_create_application_creates_formatted_reference(
    ...
    """GIVEN We have a functioning Application Store API WHEN we try to create an
    application THEN a correctly formatted reference is created for the
    application."""
...
def test_create_application_creates_unique_reference(
    ...
    """GIVEN We have a functioning Application Store API WHEN we try to create an
    application THEN a unique application_reference is created for the
    application."""
    create_application_json = {
        "account_id": "usera",
        "fund_id": "47aef2f5-3fcb-4d45-acb5-f0152b5f03c4",
...
def test_get_all_applications(flask_test_client, app):
    """GIVEN We have a functioning Application Store API WHEN a request for
    applications with no set params THEN the response should return all
    applications."""
...
def test_get_applications_of_account_id(flask_test_client, seed_application_records, unique_fund_round, app):
    """GIVEN We have a functioning Application Store API WHEN a request for
    applications of account_id THEN the response should return applications of the
    account_id."""
    with app.app_context():
        expected_data_within_response(
            flask_test_client,
            "/apply/applications?account_id=unique_user",
...
def test_update_section_of_application(flask_test_client, seed_application_records):
    """GIVEN We have a functioning Application Store API WHEN A put is made with a
    completed section THEN The section json should be updated to match the PUT'ed
    json and be marked as in-progress."""
    section_put = {
...
def test_update_section_of_application_with_incomplete_answers(flask_test_client, seed_application_records):
    """GIVEN We have a functioning Application Store API WHEN A put is made with a
    completed section THEN The section json should be updated to match the PUT'ed
    json and be marked as complete."""
    section_put = {
        "questions": test_question_data,
        "metadata": {
            "application_id": str(seed_application_records[0].id),
...
def test_get_application_by_application_id(flask_test_client, seed_application_records):
    """GIVEN We have a functioning Application Store API WHEN a GET
    /applications/<application_id> request is sent THEN the response should
    contain the application object."""
...
def test_get_application_by_application_id_and_with_qa_file(flask_test_client, seed_application_records):
    ...
    expected_data_within_response(
        flask_test_client,
        f"/apply/applications/{id}?with_questions_file=true",
        expected_data,
        exclude_regex_paths=key_list_to_regex(["reference", "started_at", "project_name", "forms"]),
        # Lists are annoying to deal with in deepdiff
        # especially when they contain dicts...so in this
...
def test_get_application_by_application_id_when_db_record_has_no_language_set(
    ...
    """GIVEN We have a functioning Application Store API WHEN a GET
    /applications/<application_id> request is sent for an application with no
    language set (such as a pre-language functionality application) THEN the
    response should contain the application object with a default language of
    english ('en')"""
    response = flask_test_client.get(
        f"/apply/applications/{seed_application_records[0].id}",
        follow_redirects=True,
    )
...
def test_get_application_by_application_id_when_db_record_has_no_language_set_and_with_qa_file(
    ...
    english ('en')"""
    response = flask_test_client.get(
        f"/apply/applications/{seed_application_records[0].id}?with_questions_file=true",
        follow_redirects=True,
    )
    response_content = json.loads(response.content)
...
def test_update_section_of_application_changes_last_edited_field(flask_test_client, seed_application_records):
    """GIVEN We have a functioning Application Store API WHEN A put is made with a
    completed section THEN the section json.last_edited should be updated."""
...
def test_update_section_of_application_does_not_change_last_edited_field(flask_test_client, seed_application_records):
    """GIVEN We have a functioning Application Store API WHEN A put is made with a
    completed section THEN The section json.last_edited should not be updated."""
...
def test_update_project_name_of_application(flask_test_client, seed_application_records):
    """GIVEN We have a functioning Application Store API WHEN a put is made into
    the 'project information' section containing a project name field THEN the
    project name should be updated on the application."""
...
def test_form_data_save_with_closed_round(flask_test_client, seed_application_records, mocker):
    ...
    and if the round is closed then it will give a 301 to redirect the
    user to fund closure notification page

    """
    mocker.patch("db.queries.apply.application.queries.get_round", new=generate_mock_round_closed)
    mocker.patch("db.queries.apply.statuses.queries.get_round", new=generate_mock_round_closed)
    section_put = {
        "questions": test_question_data,
        "metadata": {
            "application_id": str(seed_application_records[0].id),
...
def test_put_returns_400_on_submitted_application(flask_test_client, _db, seed_application_records):
    """GIVEN We have a functioning Application Store API WHEN A there is an
    application with a status of SUBMITTED THEN any PUTs to the application data
...
def test_successful_submitted_application(
    ...
    """GIVEN We have a functioning Application Store API WHEN an application is
    submitted THEN a 201 response is received in the correct format."""
    with mock.patch("apply.api.routes.application.routes.ApplicationsView._get_sqs_client") as mock_get_sqs_client:
        mock_get_sqs_client.return_value = _mock_aws_client()
        mocker.patch("db.queries.apply.application.queries.list_files_by_prefix", new=lambda _: [])
        seed_application_records[0].status = "SUBMITTED"
        ...
        response = flask_test_client.post(
            f"/apply/applications/{seed_application_records[0].id}/submit",
            follow_redirects=True,
        )
...
def test_stage_unsubmitted_application_to_queue_fails(
    ...
    """GIVEN We request to stage an unsubmitted application to the assessment
    queue WHEN an application is unsubmitted THEN a 400 response is received in
    the correct format."""
    mocker.patch("db.queries.apply.application.queries.list_files_by_prefix", new=lambda _: [])
    ...
    # mock successful notification
    response = flask_test_client.post(
        f"/apply/queue_for_assessment/{seed_application_records[0].id}",
        follow_redirects=True,
    )
...
def test_stage_submitted_application_to_queue_fails(
    ...
    WHEN an application is submitted THEN a 201 response is received in the
    correct format."""
    with mock.patch("apply.api.routes.queues.routes.QueueView._get_sqs_client") as mock_get_sqs_client:
        mock_get_sqs_client.return_value = _mock_aws_client()
        mocker.patch("db.queries.apply.application.queries.list_files_by_prefix", new=lambda _: [])
        seed_application_records[0].status = "SUBMITTED"
...
def test_post_research_survey_data(flask_test_client, mocker):
    ...
    mock_upsert_research_survey_data = mocker.patch(
        "apply.api.routes.application.routes.upsert_research_survey_data", return_value=retrieved_survey_data
    )
    mock_update_statuses = mocker.patch("apply.api.routes.application.routes.update_statuses")

    response = flask_test_client.post(
        "/apply/application/research", data=json.dumps(request_data), headers={"Content-Type": "application/json"}
    )
...
def test_get_research_survey_data(flask_test_client, mocker):
    ...
    mock_retrieve_research_survey_data = mocker.patch(
        "apply.api.routes.application.routes.retrieve_research_survey_data", return_value=retrieved_survey_data
    )

    response = flask_test_client.get(f"/apply/application/research?application_id={application_id}")
...
def test_get_research_survey_data_not_found(flask_test_client, mocker):
    ...
    mock_retrieve_research_survey_data = mocker.patch(
        "apply.api.routes.application.routes.retrieve_research_survey_data", return_value=None
    )
