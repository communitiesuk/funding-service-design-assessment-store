...
import pytest
from db.queries.apply.statuses.queries import _determine_question_page_status_from_answers
from db.queries.apply.statuses.queries import _is_all_sections_feedback_complete
from db.queries.apply.statuses.queries import _is_feedback_survey_complete
from db.queries.apply.statuses.queries import _is_field_answered
from db.queries.apply.statuses.queries import _is_research_survey_complete
from db.queries.apply.statuses.queries import update_application_status
from db.queries.apply.statuses.queries import update_form_status
from db.queries.apply.statuses.queries import update_question_page_statuses
from services.apply.models.round import FeedbackSurveyConfig
...
def test_update_question_statuses_with_mocks(mocker):
    mock_question_status = mocker.patch(
        "db.queries.apply.statuses.queries._determine_question_page_status_from_answers",
        return_value="NOT_STARTED",
    )
    mock_answer_status = mocker.patch("db.queries.apply.statuses.queries._determine_answer_status_for_fields")
...
def test_is_all_sections_feedback_complete(mocker, app_sections, feedback_for_sections, exp_result):
    mocker.patch(
        "db.queries.apply.statuses.queries.get_application_sections",
        return_value=app_sections,
    )
    mocker.patch(
        "db.queries.apply.statuses.queries.get_feedback",
...
def test_is_feedback_survey_complete(mocker, end_survey_data, exp_result):
    mocker.patch(
        "db.queries.apply.statuses.queries.retrieve_end_of_application_survey_data",
...
def test_is_research_survey_complete(mocker, research_survey_data, exp_result):
    mocker.patch(
        "db.queries.apply.statuses.queries.retrieve_research_survey_data",
...
def test_update_application_status(
    ...
    mock_fb = mocker.patch(
        "db.queries.apply.statuses.queries._is_all_sections_feedback_complete",
        return_value=feedback_complete,
    )
    mocker.patch(
        "db.queries.apply.statuses.queries._is_feedback_survey_complete",
        return_value=survey_complete,
    )
    mocker.patch(
        "db.queries.apply.statuses.queries._is_research_survey_complete",
