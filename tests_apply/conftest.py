...
from db.queries.apply.application import create_application
from db.queries.apply.form import add_new_forms
from flask import Response
from services.apply.models.fund import Fund
from services.apply.models.fund import Round
from tests_apply.helpers import APPLICATION_DISPLAY_CONFIG
from tests_apply.helpers import local_api_call
from tests_apply.helpers import test_application_data
from tests_apply.helpers import test_question_data
from tests_apply.helpers import test_question_data_cy
...
def app():
    """Creates the test client we will be using to test the responses from our
    app, this is a test fixture.
...
def unique_fund_round(mock_get_fund, mock_get_round):
    ...
    Requests mock_get_fund and mock_get_round so when the app looks up
    fund/round data it matches with these.
...
def create_app_with_blank_forms(app_to_create: dict) -> Applications:
    ...
    project-info, org-info, or their welsh equivalents depending on
    language).
...
def seed_application_records(
    ...
    """Inserts application data on a per-test (function scoped) basis to prevent
    test pollution. Provides the inserted records to tests so they can access
    them.
...
def add_org_data_for_reports(application, unique_append, client):
    """Adds additional form data to the application so it is present for testing
    the reports.

    Each org name is unique in the format 'Test Org Name
    {unique_append}'
...
def seed_data_multiple_funds_rounds(
    ...
    """Alternative to seed_application_records above that allows you to specify a
    set of funds/rounds and how many applications per round to allow testing of
    reporting functions. Expects to find fund/round config as a marker named
    'fund_round_config' in the format: {funds: [rounds: [{applications:
    [{app_data}]}]]}
...
def mock_get_fund(mocker):
    ...
    Used with unique_fund_round to ensure when the fund and round are
    retrieved, they match what's expected

    """
    mocker.patch("apply.api.routes.application.routes.get_fund", new=generate_mock_fund)
    mocker.patch("db.queries.apply.application.queries.get_fund", new=generate_mock_fund)
...
def mock_get_round(mocker):
    """Generates a mock round with the supplied fund and round IDs Used with
    unique_fund_round to ensure when the fund and round are retrieved, they match
    what's expected."""
    mocker.patch("db.queries.apply.application.queries.get_round", new=generate_mock_round)
    mocker.patch("db.queries.apply.statuses.queries.get_round", new=generate_mock_round)
    mocker.patch(
...
def mock_successful_submit_notification(mocker):
    # mock the function in the file it is invoked (not where it is declared)
    mocker.patch(
        "apply.api.routes.application.routes.Notification.send",
...
def mock_get_fund_data(mocker):
    mocker.patch(
        "apply.api.routes.application.routes.get_fund",
        return_value=Fund(
            name="COF",
            short_name="COF",
