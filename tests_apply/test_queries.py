...
from db.queries.apply.application import process_files
...
from services.apply.aws import FileData
from services.apply.models.fund import Fund
from tests_apply.seed_data.application_data import expected_application_json
...
def test_create_application_language_choice(mocker, fund_supports_welsh, requested_language, exp_language):
    ...
    mocker.patch("db.queries.apply.application.queries.get_fund", return_value=mock_fund)
    mock_create_app_try = mocker.patch(
        "db.queries.apply.application.queries._create_application_try",
...
def test_process_files(application, all_application_files, expected):
    """GIVEN an application object and a list of all files belonging to that
    application WHEN the process_files function is invoked with these parameters
    THEN the application object is expected to be updated with the relevant file
    information."""
